...
{
    [Command("build", Cmdlet = "Invoke-Traneleon")]
    ...
    public sealed class CompileCommand : ICommand
    {
        ...
        public int Execute()
        {
            if (!File.Exists(ConfigFile)) throw new FileNotFoundException($"Could not find {nameof(Traneleon)} configuration file at '{ConfigFile}'.");
